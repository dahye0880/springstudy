...
<script type="text/javascript">
   $(document).ready(function(){
	 $("#minfo").show(); //태그중 id 속성의 값이 minfo인 태그를 보이기
	 $("#oinfo").hide();//태그중 id 속성의 값이 oinfo인 태그를감추기
   ...
   function disp_div(id,tab) {  //minfo, tab1
	   //.each : 반복문
	   $(".info").each(function() {//태그중 class 속성의 값이 info인 태그들을 반복해서 감추기 
		   $(this).hide();//감추기
	   })
	   $(".tab").each(function() {
		   $(this).removeClass("select");//select 클래스 속성을 제거
	   })	   
	   $("#"+id).show();
	   $("#" + tab).addClass("select");	   
   }
